...
List *operationList = NIL;

/* V set for removal of distinct clause*/

char* V_relname[50][50];
char* V_attrname[50][50];
int V_size = 0;
...
static Query* optimizeForDistinct(Query *parse);
static bool populateScalarExpressions(Node *expr);
static bool exprIsNullConstant(Node *arg);
static List *getAttrName(Var *var, Query *query);

/*Insert in V set */
bool insertV(char* relname,char* relattr)
{
	int i = 0;
	for(i=0;i<V_size;i++)
	{
	if(strcmp(relattr,V_attrname[i])==0)
	break;
	}
	if(i==V_size)
	{
	strcpy(V_relname[V_size],relname);
	strcpy(V_attrname[V_size],relattr);
...
bool presentV(char* relname,char* relattr)
{
	int i = 0;
	for(i=0;i<V_size;i++)
	{
...
attnum = var->varattno;
refname = (char *) malloc(32);
strcpy(refname, rte->eref->aliasname);
attname = get_rte_attribute_name(rte,attnum);
rel_att = lappend(rel_att,refname);
rel_att = lappend(rel_att,attname);
return rel_att;
}
...
{
//printf("\n Node type is T_OpExpr\n");
operationList = lappend(operationList,expr);
return true;
}
...
//printf("\n Node type is T_BoolExpr\n");
lexpr = linitial(boolExpr->args);
rexpr = lsecond(boolExpr->args);
//printf("\n Left Node type is %d\n", nodeTag(lexpr));
//printf("\n Right Node type is %d\n", nodeTag(rexpr));
if (!populateScalarExpressions(lexpr))
return false;
if (!populateScalarExpressions(rexpr))
return false;
return true;
}
else
...
char* getRelName(Query* qry,Oid reloid)
{


	ListCell *lp;

	foreach(lp, qry->jointree->fromlist)
		{
		RangeTblRef *rtr = (RangeTblRef *) lfirst(lp);
...
const char* getcmdName(enum CmdType c)
{
   switch (c)
   {
...
static void insertConstExprinVset(Query *parse)
{
ListCell *lp;
Query *mod_qry = (Query *)copyObject(parse);
Node *expr = mod_qry->jointree->quals;
bool opResult;
OpExpr *opexpr;
char *op;
List *args;
char *leftattrname;
char *rightattrname;
foreach(lp, mod_qry->jointree->fromlist)
{
RangeTblRef *rtr = (RangeTblRef *) lfirst(lp);
if (IsA(rtr, RangeTblRef))
{
RangeTblEntry *rte = rt_fetch(rtr->rtindex, mod_qry->rtable);
//printf("\nTable name %s \n", rte->eref->aliasname );
}
}

opResult = populateScalarExpressions(expr);

if (opResult)
{
foreach(lp, operationList)
{
opexpr = (OpExpr *)(lfirst(lp));
op = (char *)(get_simple_binary_op_name(opexpr));
//printf("\n Operator : %s", op);
...
if (args->length == 2)
{
Node *lnode = linitial(args);
Node *rnode = lsecond(args);
char* attrname;
char* relname;
bool flag = false;
//We will use these to find the names of left and right args of = operator
if(nodeTag(lnode) == T_Var)
{
	Var *var = (Var *)lnode;
	char *refname;
	List *rel_att = getAttrName(var,parse);
	refname = linitial(rel_att);
	leftattrname = (char *)lsecond(rel_att);
	//printf("\nLeft Attr name %s", leftattrname);
	//printf("\nLeft Attr relation %s\n", refname);
	attrname = leftattrname;
	relname=refname;
}
else if(nodeTag(lnode) == T_Const)
{
	//printf("\nLeft is constant");
	flag = true;
}
...
if(nodeTag(rnode) == T_Var)
{
	Var *var = (Var *)rnode;
	char *refname;
	List *rel_att = getAttrName(var,parse);
	refname = linitial(rel_att);
	rightattrname = (char *)lsecond(rel_att);
	//printf("\nRight Attr name %s", rightattrname);
	//printf("\nRight Attr relation %s\n", refname);
	if(flag == true)
	insertV(refname,rightattrname);
}
else if(nodeTag(rnode) == T_Const)
{
	//printf("\nRight is constant\n");
	if(flag == false) // We check this to avoid clauses like const = const
	insertV(relname,attrname);
}
else
{
	//printf("\nNot supported\n");
...
void transitiveClosure(Query* parse)
{
	ListCell *lp;
	Query *mod_qry = (Query *)copyObject(parse);
	Node *expr = mod_qry->jointree->quals;
	...
	// Insert in V set
	if(presentV(lrel,lattr) || presentV(rrel,rattr))
	{
		bool f1 = insertV(lrel,lattr);
		bool f2 = insertV(rrel,rattr);
		change = change || f1;
		change = change || f2;
...
/*
 * A V set will consist of
 * 1. Projection attributes
 * 2.Attributes in conditions v = constant
 * 3. Transitive closure of the above set using AND clauses
 *
 */
void constructVset(Query* qry)
{
	V_size = 0;
// Add Projection attributes
	List	   *result = NIL;
		...
		foreach(slitem, qry->distinctClause)
		{
			SortGroupClause *scl = (SortGroupClause *) lfirst(slitem);
			TargetEntry *tle = get_sortgroupclause_tle(scl, qry->targetList);
			//printf("Target rel name %s attr name %s \n",getRelName(qry,tle->resorigtbl),tle->resname);
			insertV("NULL",tle->resname);
		}

// Add v for every clause v = constant

		insertConstExprinVset(qry);


//Compute transitive closure
		transitiveClosure(qry);
	//Printing set after closure
		printV();
}
...
bool checkPrimary(Oid relid,RangeTblRef *rtre)
{
	bool res = true;
	//printf("Printing primary");
	bool		result = false;
		...
		while (HeapTupleIsValid(tuple = systable_getnext(scan)))
			{
				...
				if (con->contype == CONSTRAINT_PRIMARY)
					{//printf("Primary key");
					//printf("Rel id passes %d retrived %d",relid,con->conrelid);
					//printf("Cols %d",(sizeof(con->conkey)/sizeof(con->conkey[1])));
					int16 t = con->conkey[1];
					//printf("Col name and nos %d",t);
					//col_list = con->conkey;
					adatum = heap_getattr(tuple, Anum_pg_constraint_conkey,
							...
							//printf("no of Cols %d",numkeys);


							for (i = 0; i < numkeys; i++)
							{
								AttrNumber	attnum = attnums[i];
								//printf("Cols no %d",attnum);
								char* attrname = get_rte_attribute_name(rtre,attnum);
								//printf("Attr name %s \n",attrname);
								//check if it is present in V set
								bool r1 = presentV("NULL",attrname);
								res = res && r1;
 ...
 removeDistinct(Query* qry)
{

	// Construct V set
	constructVset(qry);
	printV();
	// Check is all primary keys belong to V set
	...
	foreach(lp, qry->jointree->fromlist)
		...
		RangeTblRef *rtr = (RangeTblRef *) lfirst(lp);
			...
			//printf("\nTable name %s \n", rte->eref->aliasname );
			//printf("Printing primary caller");
			int16* atts;

			bool res = checkPrimary(rte->relid,rte);
			result = result && res;
	 ...
	 if(result==true)
	 {
		 printf("Distinct can be removed \n");
		 // Removing distinct
		 qry->distinctClause = NULL;
		 qry->hasDistinctOn = false;
	 }
	 else
	 {
		 printf("Distinct cannot be removed \n");
	 }
}
/*****************************************************************************
 *
...
planner(Query *parse, int cursorOptions, ParamListInfo boundParams)
{
	...
	PlannedStmt *result;
	// Check if distinct can be removed
	removeDistinct(parse);
